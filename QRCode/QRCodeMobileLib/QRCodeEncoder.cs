...
//PocketPC version		
            ...
            {
                System.Diagnostics.Debug.WriteLine("Exception for filename: '" + fileName + "'");
			...
			try
			{
                fileName = "qrvfr" + Convert.ToString(qrcodeVersion);
                System.Diagnostics.Debug.WriteLine("Trying to read: '" + fileName + "'");
		...
		private static sbyte[] divideDataBy8Bits(int[] data, sbyte[] bits, int maxDataCodewords)
		{
            ...
            if (maxDataCodewords < l2)
                throw new Exception("Can't fit data into this size");
		...
		private static sbyte[] calculateRSECC(sbyte[] codewords, sbyte rsEccCodewords, sbyte[] rsBlockOrder, int maxDataCodewords, int maxCodewords)
		{
			...
			{
			    //resource based reading
                fileName = "rsc" + rsEccCodewords.ToString();
                System.Diagnostics.Debug.WriteLine("Trying to read: '" + fileName + "'");
                MemoryStream memoryStream = new MemoryStream((byte[])Resources.ResourceManager.GetObject(fileName));
                StreamReader reader = new StreamReader(memoryStream);
				for (int i = 0; i < 256; i++)
        ...
        /// <param name="encoding"></param>
        /// <returns></returns>
        public virtual Bitmap Encode(String content, Encoding encoding, bool bUseWhiteSpace)
        {
            if (!bUseWhiteSpace)
                return this.Encode(content, encoding);

            bool[][] matrix = calQrcode(encoding.GetBytes(content));
            SolidBrush brush = new SolidBrush(qrCodeBackgroundColor);
            // white pixels plus 1 pixel for border
            int pixelesBlancos = (qrCodeScale * 4) + 1;
            // add the white pixels to the width and height
            Bitmap image = new Bitmap((matrix.Length * qrCodeScale) +  (pixelesBlancos * 2), (matrix.Length * qrCodeScale) + (pixelesBlancos * 2));
            Graphics g = Graphics.FromImage(image);
            g.FillRectangle(brush, new Rectangle(0, 0, image.Width, image.Height));
            // Draw border
#if PocketPC
            g.DrawRectangle(new Pen(qrCodeForegroundColor), new Rectangle(0, 0, image.Width - 1, image.Height - 1));
#else
            //CF does not know SolidBrush Pen
            g.DrawRectangle(new Pen(new SolidBrush(qrCodeForegroundColor)), new Rectangle(0, 0, image.Width - 1, image.Height - 1));
#endif
            brush.Color = qrCodeForegroundColor ;
            for (int i = 0; i < matrix.Length ; i++)
            {
                for (int j = 0; j < matrix.Length ; j++)
                {
                    if (matrix[j][i])
                    {
                        // Draw code beyond the white space and border
                        g.FillRectangle(brush, j * qrCodeScale + pixelesBlancos, i * qrCodeScale + pixelesBlancos, qrCodeScale, qrCodeScale);
                    }
                }
            }
            return image;
        ...
        public virtual Bitmap Encode(String content)
        {
            if (QRCodeUtility.IsUniCode(content))
            {
                ...
                return Encode(content, Encoding.UTF8);      // see http://www.codeproject.com/Messages/3994528/Re-Decode-incorrectly-when-encode-with-unicode-str.aspx
            }
            else
            {
